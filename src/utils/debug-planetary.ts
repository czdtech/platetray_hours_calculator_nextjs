...
async function debugPlanetaryHours() {
    ...
    console.log('测试日期:', formatInTimeZone(testDate, timezone, 'yyyy-MM-dd HH:mm:ss'));
    ...
    result.planetaryHours.forEach((hour, index) => {
        console.log(`${index + 1}. ${hour.ruler} (${hour.type}): ${formatInTimeZone(hour.startTime, timezone, 'HH:mm')} - ${formatInTimeZone(hour.endTime, timezone, 'HH:mm')}`);
    });
}
