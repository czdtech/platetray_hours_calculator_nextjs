...
export async function generateMetadata({
  ...
  const canonical = `${siteConfig.url}/pt/blog/category/${category}`;
...
export default async function PortugueseBlogCategoryPage({
  params,
}: {
  params: Promise<{ category: string }>;
}) {
  ...
  const label = messages.categories[category];
  const description = messages.blog.categoryDescriptions[category];
  const filtered = blogPostsPt.filter((post) => post.category === category);

  const breadcrumbItems = [
    { name: messages.common.home, url: "/pt" },
    { name: messages.common.blog, url: "/pt/blog" },
    { name: label, url: `/pt/blog/category/${category}` },
  ];
    ...
    <>
      <JsonLd
        data={getBreadcrumbSchema([
          ...
          { name: label, url: `${siteConfig.url}/pt/blog/category/${category}` },
        ])}
      />
            ...
            {filtered.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filtered.map((post) => (
                  ...
                  >
                    <Link href={`/pt/blog/${post.slug}`} className="block">
