...
export async function GET(request: Request) {
  ...
  let apiUrl = 'https://maps.googleapis.com/maps/api/geocode/json?key=' + GOOGLE_MAPS_API_KEY;
  ...
  try {
    ...
    const data = await googleResponse.json() as GeocodeSuccessResponse | GeocodeErrorResponse;

    if (!googleResponse.ok || !('results' in data && data.status === 'OK')) {
      const errorStatus = (data as GeocodeErrorResponse).status || 'Unknown API error';
      const errorMessage = (data as GeocodeErrorResponse).error_message || 'No additional details from API.';
      console.error('Google Geocoding API error. Status:', errorStatus, 'Message:', errorMessage, 'Full response:', data);
