...
export async function GET(request: Request) {
  ...
  if (!input || typeof input !== 'string') {
    return NextResponse.json({ error: 'Input parameter is required and must be a string' }, { status: 400 });
  }
  ...
  try {
    ...
    if (!googleResponse.ok || data.status !== 'OK') {
      console.error('Google Places Autocomplete API error. Status:', data.status, 'Message:', data.error_message, 'Full response:', data);
