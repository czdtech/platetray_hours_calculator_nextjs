...
export async function GET(request: Request) {
  ...
  if (!placeid || typeof placeid !== 'string') {
    return NextResponse.json({ error: 'Place ID parameter is required and must be a string' }, { status: 400 });
  }
  ...
  try {
    ...
    if (!googleResponse.ok || data.status !== 'OK') {
      console.error('Google Places Details API error. Status:', data.status, 'Message:', data.error_message, 'Full response:', data);
