...
### Project overview
Planetary Hours Calculator — a Next.js 15 App Router project deployed on Vercel. Uses TailwindCSS, TypeScript, and SunCalc for astronomical calculations. No database required for core functionality (Vercel KV used optionally for caching).

### i18n architecture
- Lightweight custom route-based i18n (no `next-intl`)
- Translation files in `src/i18n/messages/{en,es,pt}.json`
- Config in `src/i18n/config.ts`, helpers in `src/i18n/getMessages.ts`
...
- **Build:** `npm run build` (runs `prebuild` for blog metadata first)
- **Unit tests:** `npm run test` (vitest)
- **Dev server:** `npm run dev` (do NOT run — user runs it themselves)

### Gotchas
- The `prebuild` script (`npm run generate:blog-metadata`) must run before `npm run build`; it is wired as a `prebuild` hook in `package.json` so it runs automatically.
- `output: 'standalone'` is set in `next.config.ts` for Vercel deployment.
- The service worker (Serwist) is disabled in development mode automatically.
- Build produces a `MODULE_TYPELESS_PACKAGE_JSON` warning for `planetColors.shared.js` — this is a pre-existing issue, not harmful.
- City data lives in `src/data/cities.ts` (~101 cities). All localized city pages use `generateStaticParams()` from the same source.
